...
data_final <- read_excel("berkeley_raw.xls") 
...
tenuretracknot <- c("Teaching-Track")
...
private <- c("Analysis Group", "Anaylsis Group", "Uber", "Amazon", "Facebook", "Air BnB", 
             ...
             "Analytics", "True North Managers LLP", "Black Rock")
...
data_final$name <- NULL # remove ID column and generate empty ID variable 
data_final$name <- "" 

order <- c("year", "name", "placement", "placement_type") # rearrange columns
data_final <- data_final[,order]

write_xlsx(data_final, "berkeley.xls")
