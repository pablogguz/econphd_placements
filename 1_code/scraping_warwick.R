...
# Load data ---- 
url <- "https://warwick.ac.uk/fac/soc/economics/staff/job-market-candidates/placements"
web <- read_html(url)

tables <- web %>% html_nodes("table")
tables <- tables[1:8] # first two years are problematic
...
for (i in seq_along(tables)) {
  ...
  if (!is.null(table) && nrow(table[[1]]) > 1) {
    table_data <- table[[1]][, 1:3] # Keep only the first two columns
...
# Save ----
write_xlsx(final_data, paste0(data, "/eu/raw/warwick_raw.xlsx"))
